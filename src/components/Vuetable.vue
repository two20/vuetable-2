<template>
    <div>
      ...
      <select v-model="selectedGroup">
        <option v-for="group in groupings" :value="group">{{group.title}}</option>
      </select>
      </div>
    <table :class="['vuetable', css.tableClass]">
        ...
        <tbody v-cloak class="vuetable-body">
            <template v-if="grouping">
                <template v-for="(grpItem,grpIndex) in groups">
                    <tr>
                        <td :colspan="countVisibleFields" v-html="grpItem.display"></td>
                    </tr>
                    <template v-for="(item, index) in grpItem.data">
...
export default {
  ...
  computed: {
    groups(){
        ...
        let g1 = {}
        let g =  _.groupBy(this.tableData,this.grouping.name)
        _.each(g,function(element,index,list){

          g1[index] = {}
          if(self.hasCallback(self.grouping))
          {

            g1[index].display = self.callCallbackGroup(self.grouping,element)
          }
          else
          {
            g1[index].display = "BYE"
          }
          //g1[index].display = "HELLO"
          g1[index].data = element
        });
        return g1
    },
    groupingFields(){
      let gfields=  _.filter(this.tableFields,function(field){
        if(field.grouping)
          return true
        return false
      });
      if(gfields.length == 0)
        return gfields
      gfields.unshift({name:"none",title:"None"})
      return gfields
    },
    useDetailRow () {
  ...
  methods: {
      ...
      this.fields.forEach(function(field, i) {
        if (typeof (field) === 'string') {
          ...
          obj = {
            ...
            grouping: (field.grouping === undefined) ? null : field.grouping
          }
        }
    ...
    getAllQueryParams () {
      ...
      if(this.grouping)
      {
        params['group'] = this.grouping.name
      }
    ...
    callCallbackGroup (field, list) {
      ...
      if (typeof this.$parent[func] === 'function') {
        return this.$parent[func].call(this.$parent,field,list)
      }
      return null;
      /*let args = field.callback.split('|')
      let func = args.shift()
