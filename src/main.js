...
let vm = new Vue({
  ...
  methods: {
    ...
    genderGroup(group,list){
      let first = list[0]
      let salary =  _.reduce(list,function(memo,element){ return memo+parseFloat(element.salary)},0)
      return '<strong>' + group.title + ' :</strong>' + first.gender + '( ' + list.length + ' )' + salary
    },
    groupCallback(group,list){
      let first = list[0]
      return '<strong>' + group.title + ' :</strong>' + first.group.name + '( ' + list.length + ' )'
    },
    ageCallback(group,list){
      let first = list[0]
      return '<strong>' + group.title + ' :</strong>' + first.age + '( ' + list.length + ' )'
    },
    showDetailRow (value) {
